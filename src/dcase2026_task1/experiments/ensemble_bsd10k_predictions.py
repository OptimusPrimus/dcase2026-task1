...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Average BSD10k logits from one or more training runs and evaluate the "
            "ensemble on the same validation/test split used during training."
        )
    )
    parser.add_argument(
        "models",
    ...
    parser.add_argument(
        ...
        help="Prediction npz filename inside each model directory.",
    )
    parser.add_argument(
        "--output-json",
        default=None,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.output_json is not None:
        write_json(Path(args.output_json).expanduser(), results)
    return results
