...
class CutInputIntoSegmentsWrapper(nn.Module):
    def __init__(
        ...
        Args:
            ...
            min_input_length (int): Minimum length passed to the wrapped model.
        """
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        if input_length <= self.max_input_length:
            if input_length < self.min_input_length:
                x = F.pad(x, (0, self.min_input_length - input_length))
            return self.model(x).unsqueeze(1)  # Add segment dimension

        # Split into overlapping segments
        segments = []
        segment_input_length = max(self.segment_length, self.min_input_length)
        indices = list(range(0, input_length, self.hop_size))
        for i in indices:
            segment = x[:, i:i + self.segment_length]
            if segment.shape[-1] < segment_input_length:
                segment = F.pad(segment, (0, segment_input_length - segment.shape[-1]))
            segments.append(segment)

        segments = torch.stack(segments)  # Shape: (num_segments, batch_size, segment_input_length)
        outputs = self.model(segments.reshape(-1, segment_input_length))  # Process each segment
